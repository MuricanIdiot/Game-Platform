...
//magneto
function Magneto(pos){
  this.pos = pos;
  this.size = new Vector(1, 1);
  this.speed = new Vector (4, 0);
  }
Magneto.prototype.type = "magneto";

function Sabertooth(pos){
this.pos = pos;
this.size = new Vector(1, 1);
this.speed = new Vector (4, 0);
}
Sabertooth.prototype.type = "sabertooth";
...
Sabertooth.prototype.act = function(step, level) {
  var newPos = this.pos.plus(this.speed.times(step));
  if (!level.obstacleAt(newPos, this.size))
    this.pos = newPos;
  else if (this.repeatPos)
...
Magneto.prototype.act = function(step, level) {
  var newPos = this.pos.plus(this.speed.times(step));
  if (!level.obstacleAt(newPos, this.size))
    this.pos = newPos;
  else if (this.repeatPos)
...
Trig.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  else if (type == "magneto" && this.status == null) {
    this.status = "lost";
    this.finishDelay = .6; }

    else if (type == "sabertooth" && this.status == null) {
      this.status = "lost";
      this.finishDelay = .6; }
  ...
  // If there aren't any ghosts left, player looses
  if (!this.actors.some(function(actor) {
         return actor.type == "trig";
       })) {
    this.status = "lost";
    this.finishDelay = 1;
  }
